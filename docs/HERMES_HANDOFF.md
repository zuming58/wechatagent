...
- 分支：`agent/ui-demo`
- 基线提交：`0b8e2bbb7a88f2d5e4bb74f957b4015cb0e44565`
- UI Demo：`prototypes/ui-demo/`，亮蓝版已通过 `design-qa.md` 验收。
- 后端：`backend/`，FastAPI + SQLite + SQLAlchemy + Alembic + FTS5(trigram)。
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、联系人、聊天证据、原文搜索、消息上下文和用户确认事实。
...
## 6. 当前发给 Hermes 的任务：DEV-007

> 以下区块是给用户复制给 Hermes 的原文。

```text
你在 F:\WorkBuddy\wechatagent 项目中实现 DEV-007。请先阅读 docs\HERMES_HANDOFF.md，并严格遵守其中的安全与视觉边界。

任务目标：补齐用户确认事实与可选原文证据关联，仅使用已有本地消息库与合成自动化测试。事实只能由用户手动创建或确认，不得从聊天自动推断。

实施范围：
1. 新增账号隔离的事实 API：类型、内容、创建/更新时间和可选消息证据；证据仅限该联系人的私聊或其已归档群聊发言，拒绝跨联系人、跨账号和陌生群成员消息。
2. 前端实现已确认事实的创建、编辑、删除、手动记录标记和原文上下文入口；联系人切换时清除编辑状态。
