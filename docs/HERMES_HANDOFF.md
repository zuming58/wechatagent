...
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、联系人、聊天证据、原文搜索、消息上下文、用户确认事实和事实变更历史。
...
## 6. 当前发给 Hermes 的任务：DEV-008

> 以下区块是给用户复制给 Hermes 的原文。

```text
你在 F:\WorkBuddy\wechatagent 项目中实现 DEV-008。请先阅读 docs\HERMES_HANDOFF.md，并严格遵守其中的安全与视觉边界。

任务目标：补齐用户确认事实的本地变更历史，仅使用已有本地消息库与合成自动化测试。历史只能记录用户对事实的创建、编辑和删除，不得从聊天自动推断。

实施范围：
1. 新增追加式事实历史：创建、编辑、删除均保存完整本地快照与可选原文证据引用；删除当前事实后历史必须仍可读取，不提供恢复功能。
2. 新增账号隔离的历史查询 API；前端展示操作类型、内容、时间和证据上下文入口，联系人切换时清除历史与上下文状态。
