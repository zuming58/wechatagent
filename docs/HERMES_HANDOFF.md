...
## 6. 当前开发状态：DEV-022 已完成

DEV-022 新增 SQLite FTS5(trigram) 索引健康检查与显式重建。状态比较原始消息数和索引消息数；重建只重写选定账号的 `messages_fts` 条目，不修改原始消息、不调用连接器或同步。所有验证仅使用 mock API、合成数据和临时数据库；未安装、未执行或接入 `wx-cli`，未读取、解密、导入或上传任何真实微信聊天数据。
