...
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、自动增量同步状态、联系人档案、聊天证据、带结构化筛选的原文搜索、安全附件摘要、消息上下文、用户确认事实与历史、用户手动知识卡和待办事项。
- DEV-001 至 DEV-015 已完成并通过审计：同步闭环、多账号显式选择、合成采集协议、前端安全闸门、同步新鲜度、联系人证据、用户事实/资料及历史、合成自动同步、结构化搜索、知识卡、本地存储状态、手动待办，以及 Alembic 空库迁移链验证。
...
## 6. 当前开发状态：DEV-015 已完成

DEV-015 修复并验证了 Alembic 的历史迁移链：初始迁移不再引用当前运行时模型元数据，从空的临时 SQLite 数据库升级至当前 `head` 时不会提前创建未来表或与后续迁移冲突。当前没有待发的 Hermes 任务；下一项开发工作必须另行编号、明确范围并经过安全审计。
