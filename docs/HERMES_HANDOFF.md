...
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、自动增量同步状态、联系人档案、聊天证据、独立全局原文搜索、只读本地时间线、安全附件摘要、消息上下文、用户确认事实与历史、用户手动知识卡，以及具备原文证据和历史的手动待办事项。
- DEV-001 至 DEV-018 已完成并通过审计：同步闭环、多账号显式选择、合成采集协议、前端安全闸门、同步新鲜度、联系人证据、用户事实/资料及历史、合成自动同步、结构化搜索、知识卡、本地存储状态、手动待办及其证据/历史、全局搜索工作流、只读本地时间线，以及 Alembic 空库迁移链验证。
...
## 6. 当前开发状态：DEV-018 已完成

DEV-018 新增账号隔离的只读时间线 API 和工作区，将已归档原文消息、联系人资料/事实历史、本地知识卡及手动待办操作按时间聚合；可按类型和当前联系人收窄，关联原文可打开上下文。时间线不写入、推断或导出任何数据；标签管理保持禁用并标为开发中。所有验证仅使用 mock API、合成数据和临时数据库；当前没有待发的 Hermes 任务，下一项开发工作必须另行编号、明确范围并经过安全审计。
