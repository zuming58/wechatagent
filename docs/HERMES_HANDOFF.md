...
## 6. 当前发给 Hermes 的任务：DEV-012

> 以下区块是给用户复制给 Hermes 的原文。

```text
你在 F:\WorkBuddy\wechatagent 项目中实现 DEV-012。请先阅读 docs\HERMES_HANDOFF.md，并严格遵守其中的安全与视觉边界。

任务目标：补齐用户手动知识卡、原文证据关联和本地变更历史，仅使用合成消息、临时数据库和 mock API。
