...
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、自动增量同步状态、联系人档案、聊天证据、独立全局原文搜索、安全附件摘要、消息上下文、用户确认事实与历史、用户手动知识卡，以及具备原文证据和历史的手动待办事项。
- DEV-001 至 DEV-017 已完成并通过审计：同步闭环、多账号显式选择、合成采集协议、前端安全闸门、同步新鲜度、联系人证据、用户事实/资料及历史、合成自动同步、结构化搜索、知识卡、本地存储状态、手动待办及其证据/历史、全局搜索工作流，以及 Alembic 空库迁移链验证。
...
## 6. 当前开发状态：DEV-017 已完成

DEV-017 将左侧“全局搜索”实现为独立工作区，默认在当前账号全部已归档消息中查询；用户可显式收窄到当前联系人，并能从结果打开消息上下文或发起手动事实、待办。时间线和标签管理保持禁用并标为开发中，避免无反馈操作。所有验证仅使用 mock API、合成数据和临时数据库；当前没有待发的 Hermes 任务，下一项开发工作必须另行编号、明确范围并经过安全审计。
