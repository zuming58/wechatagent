...
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、自动增量同步状态、联系人档案、聊天证据、带结构化筛选的原文搜索、安全附件摘要、消息上下文、用户确认事实与历史、用户手动知识卡，以及具备原文证据和历史的手动待办事项。
- DEV-001 至 DEV-016 已完成并通过审计：同步闭环、多账号显式选择、合成采集协议、前端安全闸门、同步新鲜度、联系人证据、用户事实/资料及历史、合成自动同步、结构化搜索、知识卡、本地存储状态、手动待办及其证据/历史，以及 Alembic 空库迁移链验证。
...
## 6. 当前开发状态：DEV-016 已完成

DEV-016 为用户手动待办增加同账号原文证据和追加式本地操作历史；创建、更新、完成/恢复、删除均保存快照，删除后当前待办不可见但历史可读。当前没有待发的 Hermes 任务；下一项开发工作必须另行编号、明确范围并经过安全审计。
