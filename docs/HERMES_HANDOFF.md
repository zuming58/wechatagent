...
- DEV-001 至 DEV-021 已完成并通过审计：同步闭环、多账号显式选择、合成采集协议、前端安全闸门、同步新鲜度、联系人证据、用户事实/资料及历史、合成自动同步、结构化搜索、知识卡、本地存储状态、本地数据管理安全闸门、本地隐私确认与安全模式、手动待办及其证据/历史、全局搜索工作流、只读本地时间线、用户手动标签，以及 Alembic 空库迁移链验证。
...
## 6. 当前开发状态：DEV-021 已完成

DEV-021 新增首次本地隐私确认和可复查的安全设置。未确认本地处理边界时，前端不得调用数据源探测；确认仅保存本地处理边界，不会授权真实采集或 AI/云端处理。设置页面持续显示“真实采集未获 Go/No-Go 授权”和“AI 与云端处理已关闭”。所有验证仅使用 mock API、合成数据和临时数据库；未安装、未执行或接入 `wx-cli`，未读取、解密、导入或上传任何真实微信聊天数据。下一项开发工作必须另行编号、明确范围并经过安全审计。
