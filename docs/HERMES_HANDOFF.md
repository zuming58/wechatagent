...
- 正式前端：`frontend/`，React + TypeScript，已接通合成数据的状态、首次归档、自动增量同步状态、联系人档案、聊天证据、原文搜索、消息上下文、用户确认事实和事实变更历史。
...
## 6. 当前发给 Hermes 的任务：DEV-010

> 以下区块是给用户复制给 Hermes 的原文。

```text
你在 F:\WorkBuddy\wechatagent 项目中实现 DEV-010。请先阅读 docs\HERMES_HANDOFF.md，并严格遵守其中的安全与视觉边界。

任务目标：补齐合成数据模式下的本地自动增量同步与运行状态，仅使用现有合成连接器、临时数据库和 mock API。

实施范围：
1. 首次归档成功后，合成账号可按配置周期执行幂等增量同步；首次归档前不得自动采集。
2. 手动同步和自动同步按账号串行，重复同步保留去重、水位与新鲜度语义。
3. 提供最近同步记录与自动同步状态 API，并在前端明确展示调度范围和最近结果。
4. 自动同步只能用于合成连接器。真实连接器保持未授权、不得由后台任务探测或执行。
5. 不安装、不执行或接入 wx-cli；不读取、解密、导入或上传任何真实微信数据；不改变 DEV-002 的 POST /api/v1/sync 多账号安全闸门。
