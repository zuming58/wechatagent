...
DEV-024 在 DEV-023 的范围报告中补齐按最新活动排序的已归档会话清单：只读返回当前账号至多 12 个会话的显示名、会话类型、聚合消息数与时间范围；不返回消息正文、来源路径或账号稳定标识。DEV-025 已获用户明确的非管理员、只读 Go/No-Go 授权；预检仅确认一个匿名账号目录和 `connector_missing`，未读取聊天内容。官方 `jackwener/wx-cli` GitHub 仓库与 API 均返回 DMCA 下架，不能从官方来源安装，因而不得使用未知镜像或二进制继续。真实数据尚未读取、解密、导入或上传。
