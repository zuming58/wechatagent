...
- DEV-001 至 DEV-019 已完成并通过审计：同步闭环、多账号显式选择、合成采集协议、前端安全闸门、同步新鲜度、联系人证据、用户事实/资料及历史、合成自动同步、结构化搜索、知识卡、本地存储状态、手动待办及其证据/历史、全局搜索工作流、只读本地时间线、用户手动标签，以及 Alembic 空库迁移链验证。
...
## 6. 当前开发状态：DEV-019 已完成

DEV-019 新增账户隔离的用户手动标签与关联表。用户可创建、编辑、删除标签，并只可关联同账号当前存在的联系人、事实、知识卡或待办；标签及关联不会由同步或聊天自动生成，删除目标会清理其关联。所有验证仅使用 mock API、合成数据和临时数据库；当前没有待发的 Hermes 任务，下一项开发工作必须另行编号、明确范围并经过安全审计。
