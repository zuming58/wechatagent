...
import { api, type Contact, type Message, type SourceStatus } from "./api";
...
export function App() {
  ...
  useEffect(() => {
    if (!accountId || !sourceReady) return;
  ...
  return <div className="app-shell">
    ...
    <main>
      <header className="status"><span className={sourceReady ? "ok" : "warn"}>{sourceReady ? <CloudCheck weight="fill" /> : <WarningCircle weight="fill" />}{statusCopy}</span><button disabled={!sourceReady || syncing} onClick={() => sync("incremental")}>{syncing ? <CircleNotch className="spin" /> : <ArrowClockwise />}立即同步</button></header>
      <section className="hero"><div className="profile-avatar">{selected ? avatar(selected) : <UsersThree />}</div><div><h1>{selected?.display_name ?? "开始建立本地关系记忆"}</h1><p>{selected?.company || selected?.role || "先选择一个已确认的数据源账号，再开始首次归档。"}</p><div className="chips"><span>本地优先</span><span>原文可追溯</span><span>身份稳定</span></div></div><div className="hero-actions"><button className="secondary" onClick={() => sync("initial")} disabled={!sourceReady || syncing}>首次归档</button><button className="primary" onClick={search}><MagnifyingGlass />搜索聊天记录</button></div></section>
      <div className="tabs"><button className="active">关系总览</button><button>聊天证据</button><button>待办事项</button><button>标签与备注</button></div>
      <div className="content"><section className="card confirmed"><h2><CheckCircle weight="fill" />已确认的事实</h2><p>真实聊天同步后，在这里保存由你确认的关系事实；每一条都会保留原文上下文入口。</p><ul><li>联系人身份由账号标识与稳定内部标识绑定。</li><li>头像变化仅更新缓存，不会产生新联系人。</li><li>原始消息落库成功后才推进同步水位。</li></ul></section><section className="card ai"><h2><ChatCircleDots weight="fill" />原文搜索</h2><div className="search-results"><label className="inline-search"><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && search()} placeholder="输入关键词后按 Enter" /><button className="link" onClick={search}>搜索</button></label>{results.map((item) => <article key={item.id}><strong>{item.conversation_name} · {item.sender_display_name}</strong><p>{item.text_content}</p><small>{new Date(item.sent_at).toLocaleString("zh-CN")}</small></article>)}</div></section><section className="card needs"><h2>同步状态与下一步</h2>{source && source.accounts.length > 1 && <label className="account-picker">当前本地账号<select value={accountId} onChange={(event) => setAccountId(event.target.value)}>{source.accounts.map((account) => <option value={account.id} key={account.id}>{account.display_name}</option>)}</select></label>}<p>{notice || "使用合成数据完成一次归档后，可验证联系人、搜索与上下文。真实微信连接仍需通过只读 Go/No-Go。"}</p></section></div>
    </main>
  </div>;
}
