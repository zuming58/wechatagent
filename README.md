...
- `frontend/`：React + TypeScript 正式界面，已接入数据源状态、同步与最近运行记录、联系人档案、聊天证据、原文搜索、消息上下文、用户确认事实和事实变更历史。
...
首次启动后，在前端点击“首次归档”即可导入合成数据；这不会访问微信。首次归档成功后，默认每 5 分钟会为该合成账号执行一次幂等增量同步，页面会显示调度状态和最近同步结果。自动调度不会在真实连接器模式下运行。随后可在“聊天证据”查看联系人私聊与该联系人在已归档群聊中的发言，并从证据或关键词搜索结果打开消息上下文。关系总览中的“联系人档案”可维护本地备注、确认实名、公司和角色；这些用户覆盖值不会被同步覆盖，也不代表聊天自动结论。已确认事实仍只由用户手动创建或确认；可选关联该联系人的原文证据，也可明确标记为“用户手动记录”。同步不会推断、覆盖或删除这些事实。“事实历史”仅记录用户在本地对事实的创建、编辑和删除快照，不代表聊天自动结论。
