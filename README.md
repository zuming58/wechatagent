...
- `frontend/`：React + TypeScript 正式界面，已接入数据源状态、同步与最近运行记录、联系人档案、聊天证据、独立全局原文搜索、消息上下文、用户确认事实、事实变更历史、用户手动知识卡与待办事项。
